    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = [
          "arn:aws:s3:::tf-null-resource-test-bucket",
          "arn:aws:s3:::tf-null-resource-test-bucket/*"
        ]
      }
    ]
...
resource "aws_instance" "web_server" {
  ami                  = "ami-002f6e91abff6eb96" # Update with a valid AMI ID
...
resource "null_resource" "setup_and_upload" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "aws s3 cp /var/www/html/index.html s3://tf-null-resource-test-bucket/",
